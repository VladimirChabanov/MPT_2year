# Практическое задание 3. Знакомство с Kubernetes.
...
2. После того, как вы нажмёте на кнопку "Start" для вас будет создана 4-х часовая сессия, в пределах которой можно будет создать до 5-ти серверов (node) с уже установленными компонентами kubernetes.  
   Создайте максимум нод при помощь кнопки "Add new instance".
...
5. В данном случае мы будем создавать кластер с одной мастер-нодай (хотя можно и больше) и 4-мя рабочими нодами.  Поэтому:
   ...
   - Здесь же будет показана строка которую нужно будет запускать на рабочих узлах для подключения к кластеру. Найдите её и скопируйте в какой ни будь текстовый файл, она нам ещё пригодится. В моём случае она выглядит так:  
     ...
     Здесь просто указан ip-адрес управляющего сервера и токен доступа.
...
8. Подключим остальные сервера к кластеру.  
   Для этого на оставшихся 4-х нодах запустите команду `kubeadm join` с параметрами, которые вы копировали ранее в таксовый файл. В моём случае команда выглядит так:  
...
9. Перейдите на матер-ноду и снова выполните команду:  
   ```bash
   kubectl get nodes
   ```
...
15. Отключим одну или несколько нод на которых работают поды с "nginx".  Процесс коротконого отключения ноды от реального кластера может содержать больше шагов, чем приведены здесь:

    - Определите на каких нодах запушены поды с "nginx" (у меня это node4, node5, node2).
...
16. Остановим запущенное приложение "nginx". Для этого выполните команду запуска, но с вместо `applay` укажите `delete`, т.е.:  
    ```bash
...
Больше на этом кластере мы ничего делать не будем, т.к. в дальнейшем все команды которые мы будем запускать одинаково отрабатывают на кластере с любым количеством нод (в том числе и одной). Остальная часть работы будет породить на кластере разинутом на вашей локальной машине.

### Разворачиваем локальный кластер на основе Minikube
